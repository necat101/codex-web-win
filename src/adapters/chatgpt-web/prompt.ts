...
export function compileChatGptWebPrompt(
  ...
  const transportContract = mode.localTools
    ? [
      ...
      "If sandbox=\"dangerFullAccess\", ordinary in-scope local operations are authorized. Request an in-scope command even if its tool is annotated destructive; only the outer Codex runtime can refuse it. Honor real model-safety, native approval/policy/sandbox/OS/tool refusals.",
      ...
      "After binding, for tool tasks emit a brief task-facing commentary update before the first substantive tool call and between meaningful batches. Skip chatter for coupled polls/retries; commentary never replaces execution.",
      "Use codex_apply_patch for edits and codex_exec for commands. Never chain diagnostics with ;, &&, echo, or Write-Output. If refused, retry each command separately. Only then, if binding still advertises exec, find exec_command or shell_command with codex_tool_inventory and call via codex_tool_call before concluding local command execution is unavailable. Use codex_write_stdin only for returned session_id; otherwise use codex_exec timeout_ms.",
      "For local media, ffprobe and ffmpeg are ordinary commands when installed: probe or extract a few frames with codex_exec and inspect frames with codex_view_image. A missing binary is a missing dependency, not a permission failure unless the native tool explicitly says so.",
      "Continue from tool results and verify requested code or artifact changes with useful checks before final; report only verification that actually ran.",
