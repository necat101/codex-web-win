...
export function compileChatGptWebPrompt(
  ...
  const transportContract = mode.localTools
    ? [
      ...
      "Use codex_apply_patch for targeted edits and codex_exec first for commands. If codex_exec is refused by the client/tool layer but the binding still advertises exec, use codex_tool_inventory to find exec_command or shell_command and invoke it through codex_tool_call before concluding local command execution is unavailable. Call codex_write_stdin only when codex_exec or codex_tool_call actually returns a session_id. When the binding advertises session_termination, codex_write_stdin terminate=true can stop a yielded wait cell that is no longer needed. If codex_bind_turn capabilities omit session_history, use codex_exec timeout_ms for long blocking commands instead of treating a short yield_time_ms as a runtime deadline.",
