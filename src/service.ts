...
export function removeLegacyRuntimeArtifacts(config: AppConfig): void {
  ...
  const wrapper = normalize(legacyWrapper);
  const vendor = normalize(legacyVendor);
  if (config.runtimeCommand.some(part => {
    if (!isAbsolute(part)) return false;
    const candidate = normalize(part);
    return candidate === wrapper || candidate === vendor || candidate.startsWith(`${vendor}${sep}`);
  })) {
